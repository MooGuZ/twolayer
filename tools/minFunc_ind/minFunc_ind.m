...
if ~isempty(outputFcn)
    callOutput(outputFcn,x,'init',0,funEvals,f,[],[],g,[],double(sum(abs(g))),varargin{:});
end
...
if sum(abs(g)) <= tolFun
    ...
    if nargout > 3
        output = struct('iterations',0,'funcCount',1,...
            'algorithm',method,'firstorderopt',double(sum(abs(g))),'message',msg,'trace',trace);
    end
    return;
end
...
for i = 1:maxIter
        ...
        case PCG % Preconditioned Non-Linear Conjugate Gradient
            ...
            if isempty(precFunc)
                ...
                if i == 1
                    ...
                    [old_dirs,old_stps,Hdiag] = lbfgsUpdate_ind(g-g_old,t*d,corrections,debug,old_dirs,old_stps,Hdiag);

                    if useMex
                        s = lbfgsC(-g,old_dirs,old_stps,Hdiag);
                    else
                        s = lbfgs_ind(-g,old_dirs,old_stps,Hdiag);
                    end
                end
        ...
        case LBFGS % L-BFGS
            ...
            if i == 1
                ...
                if useMex
                    ...
                    d = lbfgs_ind(-g,old_dirs,old_stps,Hdiag);
                end
            end
            g_old = g;
    ...
    elseif LS < 6
        ...
        %g = double(g);
        %d = double(d);
        %whos x t d f g gtd c1 c2 LS tolX debug doPlot funObj varargin
        if computeHessian
    ...
    if verboseI
        fprintf('%10d %10d %15.5e %15.5e %15.5e\n',i,funEvals*funEvalMultiplier,t,f,double(sum(abs(g))));
    end

    % Output Function
    if ~isempty(outputFcn)
        callOutput(outputFcn,x,'iter',i,funEvals,f,t,gtd,g,d,double(sum(abs(g))),varargin{:});
    end
...
if verbose   
    fprintf('%10d %10d %15.5e %15.5e %15.5e   %s \n',double(i),double(funEvals*funEvalMultiplier),double(t),double(f),double(sum(abs(g))), msg);
end
if nargout > 3
...
% Output Function
if ~isempty(outputFcn)
    callOutput(outputFcn,x,'done',i,funEvals,f,t,gtd,g,d,double(sum(abs(g))),varargin{:});
end
