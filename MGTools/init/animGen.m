...
p.addParameter('frmsize',[32,32], ...
    @(x) isnumeric(x) && isreal(x) && all((x-floor(x))==0) ...
        && numel(x) <= 2);
p.addParameter('frmnumber',24, ...
    @(x) isscalar(x) && isnumeric(x) && isreal(x));
p.addParameter('objsize',10, ...
    @(x) isnumeric(x) && isreal(x) && x > 0);
p.addParameter('objorient',0, ...
    @(x) isscalar(x) && isnumeric(x) && isreal(x));
p.addParameter('objposition',nan, ...
    @(x) any(isnan(x)) || (isnumeric(x) && isreal(x) && numel(x) == 2));
p.addParameter('smoothness',1, ...
    @(x) isscalar(x) && isnumeric(x) && isreal(x) && x >= 0);
p.addParameter('mvdirection',0, ...
    @(x) isscalar(x) && isnumeric(x) && isreal(x));
p.addParameter('edgenumber',5, ...
    @(x) isscalar(x) && isnumeric(x) && isreal(x) && x - floor(x) == 0);
p.addParameter('edgedist',[], ...
    @(x) isvector(x) && isnumeric(x) && isreal(x));
p.addParameter('edgeorient',[], ...
    @(x) isvector(x) && isnumeric(x) && isreal(x));
